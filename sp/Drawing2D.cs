...
public static class Drawing2D
{
	...
	public static void DrawRect(Rect rect, Color color, float width, bool antiAlias){
		Vector2 pointA = new Vector2(rect.x, rect.y);			
		Vector2 pointB = new Vector2(rect.x+rect.width, rect.y+rect.height);
		Vector2 pointC = new Vector2(pointB.x, rect.y);
		Vector2 pointD = new Vector2(rect.x, pointB.y);
	
		DrawLine(pointA, pointC, color, width, antiAlias);
		DrawLine(pointC, pointB, color, width, antiAlias);
		DrawLine(pointB, pointD, color, width, antiAlias);
		DrawLine(pointD, pointA, color, width, antiAlias);
	}
    ...
    private static void Initialize()
    {
        ...
        if (aaLineTex == null)
        {
            // TODO: better anti-aliasing of wide lines with a larger texture? or use Graphics.DrawTexture with border ettings
